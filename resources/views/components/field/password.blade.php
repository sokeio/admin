...
@endphp
<div class=" {{ $column->getClassName() ?? 'mb-3' }}" x-data="{ showPass: false }">
    <label class="form-label">{{ $modelLabel }}</label>
    <div class="input-group input-group-flat">
        <input type="password"  :type="showPass ? 'text' : 'password'" class="form-control" name="field-{{ $modelField }}" placeholder="{{ $modelPlaceholder }}"
